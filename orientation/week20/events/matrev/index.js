...
const showError = (context, message) => {
    ...
    `; //ez csinalja a kis piros uzenetet
...
const retrieveParticipants = async () => {
    const resource = '/api/participants'; //ez a resource
    const response = await fetch(resource); //itt van az amit kert 
    const data = await response.json(); //itt atadja
...
window.addEventListener('DOMContentLoaded', () => {
    retrieveParticipants(); //ez meghivja , betolti az egesz adatot betolti

    document.querySelector('#add-button').onclick = handleAdd;// ez csak regisztralva van, de ehez meg meg kell nyomni a gombot
...
//  preventDefault - ne toltodjon be ujra az egesz oldal
// Submit es a cancel gomb csak akkor letezik mikor rakattintunk